...
plot_data_2 <- plot_data %>%
  ...
  group_by(true_subgroup_order, timepoints, states) %>%
  summarize(M = median(value))

ranks <- plot_data %>% 
  filter(type == 'rank') %>%
  filter(measure == 'phiaic') %>%
  #filter(timepoints == 50) %>%
  filter(states == 5) %>%
  #filter(true_subgroup_order == 3) %>%
  #mutate(measure = recode(measure, phiaic = "phibic, phiaic, phiaicc")) %>%
  ggplot(aes(y = value, x = true_subgroup_order)) +
  geom_boxplot(aes(fill = timepoints), outlier.shape = NA) + 
...
orders <- plot_data %>%
  ...
  #filter(states == 5) %>%
  ggplot(aes(x = true_subgroup_order, y = perc_true_order, 
             fill = states)) + 
