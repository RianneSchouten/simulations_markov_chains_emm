...
def create_starting_descriptions(df=None, cols=None, 
    ...
    cq_bin_nom = refine_nominal_attributes(cq=cq_bin, seed=None, df=df, subgroup=None, nominal_attributes=nom_atts)
...
def refine_seed(seed=None, subgroup=None, bin_atts=None, num_atts=None, nom_atts=None, dt_atts=None, nr_quantiles=None):

    cq_bin = refine_binary_attributes(seed=seed, df=None, subgroup=subgroup, binary_attributes=bin_atts)
    cq_bin_nom = refine_nominal_attributes(cq=cq_bin, seed=seed, df=None, subgroup=subgroup, nominal_attributes=nom_atts)
...
def refine_nominal_attributes(cq=None, seed=None, df=None, subgroup=None, nominal_attributes=None):
    ...
    # refinements for existing candidate queue
        ...
        for attribute in nominal_attributes:
            ...
            if not attribute in list(description.keys()) and len(values) > 1:
                for i in range(len(values)):

                    value = values[i]

                    temp_desc = description.copy()
                    temp_desc[attribute] = (1.0, value)        
                    refined_cq.append({'description' : temp_desc})

                    temp_desc = description.copy()
                    temp_desc[attribute] = (0.0, value)
                    refined_cq.append({'description' : temp_desc})
