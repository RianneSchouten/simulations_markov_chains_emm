...
def add_qm(desc=None, idx_sg=None, general_params=None, subgroup_params=None):
...
def calculate_qm(general_params=None, subgroup_params=None):

    qm_all = {}
   
    d_unw, d_w = h_distance_transition_matrix(general_params=general_params, subgroup_params=subgroup_params)
...
def calculate_general_parameters(df=None, cols=None, time_attributes=None, id_attribute=None):
    ...
    ls1, ls, lss, tA = me.t_count_matrix(df=df, time_attributes=time_attributes, states=states)
    ll_d = me.log_likelihood(df=df, time_attributes=time_attributes, id_attribute=id_attribute, states=states,
                             model_ls1=ls1, model_tA=tA, data_ls1=ls1, data_lss=lss)

    general_params = {'ls1': ls1, 'ls': ls, 'lss': lss, 'tA': tA, 'll_d': ll_d, 'states': states, 'data_size': data_size}  

    return general_params

def calculate_subgroup_parameters(df=None, subgroup=None, idx_sg=None, time_attributes=None, general_params=None, id_attribute=None):

    sg_size = len(subgroup)
    ls1, ls, lss, tA = me.t_count_matrix(df=subgroup, time_attributes=time_attributes, states=general_params['states'])

    ll_sg = me.log_likelihood(df=subgroup, time_attributes=time_attributes, id_attribute=id_attribute, states=general_params['states'], 
                              model_ls1=ls1, model_tA=tA, data_ls1=ls1, data_lss=lss)
    ll_pi_sg = me.log_likelihood(df=subgroup, time_attributes=time_attributes, id_attribute=id_attribute, states=general_params['states'], 
                              model_ls1=general_params['ls1'], model_tA=general_params['tA'], data_ls1=ls1, data_lss=lss)
    ll_sg_d = me.log_likelihood(df=df, time_attributes=time_attributes, id_attribute=id_attribute, states=general_params['states'], 
                             model_ls1=ls1, model_tA=tA, data_ls1=general_params['ls1'], data_lss=general_params['lss'])

    #mu, sigma = me.sample_dataset(df=df, size=sg_size, time_attributes=time_attributes, general_params=general_params, M=50)
    #if sigma == 0.0: sigma = 1.0
        
    subgroup_params = {'ls1': ls1, 'ls': ls, 'lss': lss, 'tA': tA, #'mu': mu, 'sigma': sigma, 
...
def h_distance_transition_matrix(general_params=None, subgroup_params=None):
    ...
    d_unw = me.manhattan_distance(taA=general_params['tA'], taB=subgroup_params['tA'], lsB=subgroup_params['ls'], weighted=False)
    d_w = me.manhattan_distance(taA=general_params['tA'], taB=subgroup_params['tA'], lsB=subgroup_params['ls'], weighted=True)
